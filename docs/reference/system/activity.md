...
```graphql
type Query {
	activity_by_id(id: ID!): sigedin_activity
}
```
...
```graphql
type Mutation {
	create_comment(collection: String!, item: ID!, comment: String!): sigedin_activity
}
```
...
```graphql
type Mutation {
	update_comment(id: ID!, comment: String!): sigedin_activity
}
```
