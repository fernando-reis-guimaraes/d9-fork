...
`id` **integer**\
Primary key of the metadata row in `sigedin_relations`.
...
```json
{
	"collection": "about_us",
	"field": "logo",
	"related_collection": "sigedin_files",
	"schema": {
		"table": "about_us",
		"column": "logo",
		"foreign_key_table": "sigedin_files",
...
```graphql
type Query {
	relations_in_collection(collection: String!): [sigedin_relations]
}
```
...
```graphql
type Query {
	relations_by_name(collection: String!, field: String!): sigedin_relations
}
```
...
```graphql
type Mutation {
	create_relations_item(data: create_sigedin_relations_input!): sigedin_relations
}
```
...
mutation {
	create_relations_item(
		data: { collection: "articles", field: "featured_image", related_collection: "sigedin_files" }
	) {
		collection
		field
		related_collection
...
```graphql
type Mutation {
	update_relations_item(collection: String!, field: String!, data: update_sigedin_relations_input!): sigedin_relations
}
```
