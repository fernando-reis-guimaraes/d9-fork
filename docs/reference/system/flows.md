...
```graphql
type Query {
	flows_by_id(id: ID!): sigedin_flows
}
```
...
```graphql
type Mutation {
	create_flows_item(data: create_sigedin_flows_input!): sigedin_flows
}
```
...
```graphql
type Mutation {
	create_flows_items(data: [create_sigedin_flows_input!]!): [sigedin_flows]
}
```
...
```graphql
type Mutation {
	update_flows_item(id: ID!, data: update_sigedin_flows_input): sigedin_flows
}
```
...
```graphql
type Mutation {
	update_flows_items(ids: [ID!]!, data: update_sigedin_flows_input): [sigedin_flows]
}
```
