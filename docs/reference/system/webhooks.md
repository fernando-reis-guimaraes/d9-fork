...
```graphql
type Query {
	webhooks_by_id(id: ID!): sigedin_webhooks
}
```
...
```graphql
type Mutation {
	create_webhooks_item(data: create_sigedin_webhooks_input!): sigedin_webhooks
}
```
...
```graphql
type Mutation {
	create_webhooks_items(data: [create_sigedin_webhooks_input!]!): [sigedin_webhooks]
}
```
...
```graphql
type Mutation {
	update_webhooks_item(id: ID!, data: update_sigedin_webhooks_input!): sigedin_webhooks
}
```
...
```graphql
type Mutation {
	update_webhooks_items(ids: [ID!]!, data: update_sigedin_webhooks_input!): [sigedin_webhooks]
}
```
