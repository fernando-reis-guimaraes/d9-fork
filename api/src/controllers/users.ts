...
router.use(useCollection('sigedin_users'));
...
const readHandler = asyncHandler(async (req, res, next) => {
	...
	const meta = await metaService.getMetaForQuery('sigedin_users', req.sanitizedQuery);
		...
		if (req.accountability.role) {
			...
			if (role && role.enforce_tfa) {
				const existingPermission = await req.accountability.permissions?.find(
					(p) => p.collection === 'sigedin_users' && p.action === 'update'
				);
