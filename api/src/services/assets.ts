...
export class AssetsService {
	...
	async getAsset(
		 ...
		 * with a wrong type. In case of sigedin_files where id is a uuid, we'll have to verify the
		...
		if (systemPublicKeys.includes(id) === false && this.accountability?.admin !== true) {
			await this.authorizationService.checkAccess('read', 'sigedin_files', id);
		}

		const file = (await this.knex.select('*').from('sigedin_files').where({ id }).first()) as File;
