...
describe('Integration Tests', () => {
	...
	beforeEach(() => {
		tracker.on.any('sigedin_roles').response({});

		tracker.on
			.select(/"sigedin_roles"."id" from "sigedin_roles" order by "sigedin_roles"."id" asc limit .*/)
			.response([]);
	});
				...
				describe('on an admin role', () => {
					...
					describe('with an array of user ids', () => {
						it('having an added user', async () => {
							...
							tracker.on.select('select "admin_access" from "sigedin_roles"').responseOnce({ admin_access });
							tracker.on.select('select "id" from "sigedin_users" where "role" = ?').responseOnce([{ id: userId1 }]);
						...
						it('having a removed user', async () => {
							...
							tracker.on
								.select('select "id" from "sigedin_users" where "role" = ?')
						...
						it('having a removed last user that is not the last admin of system', async () => {
							...
							tracker.on.select('select "admin_access" from "sigedin_roles"').responseOnce({ admin_access });
							tracker.on.select('select "id" from "sigedin_users" where "role" = ?').responseOnce([{ id: userId1 }]);
							tracker.on.select('select count(*) as "count" from "sigedin_users"').responseOnce({ count: 1 });
						...
						it('having a removed a last user that is the last admin of system', async () => {
							...
							tracker.on.select('select "admin_access" from "sigedin_roles"').responseOnce({ admin_access });
							tracker.on.select('select "id" from "sigedin_users" where "role" = ?').responseOnce([{ id: userId1 }]);
							tracker.on.select('select count(*) as "count" from "sigedin_users"').responseOnce({ count: 0 });
