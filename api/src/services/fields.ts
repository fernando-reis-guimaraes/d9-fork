...
export class FieldsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.itemsService = new ItemsService('sigedin_fields', options);
		this.payloadService = new PayloadService('sigedin_fields', options);
	...
	private get hasReadAccess() {
		return !!this.accountability?.permissions?.find((permission) => {
			return permission.collection === 'sigedin_fields' && permission.action === 'read';
		});
	}
	...
	async readAll(collection?: string): Promise<Field[]> {
		...
		const nonAuthorizedItemsService = new ItemsService('sigedin_fields', {
			knex: this.knex,
			schema: this.schema,
		});
		...
		const aliasQuery = this.knex.select<any[]>('*').from('sigedin_fields');
	...
	async readOne(collection: string, field: string): Promise<Record<string, any>> {
		...
		let fieldInfo = await this.knex.select('*').from('sigedin_fields').where({ collection, field }).first();
	...
	async createField(
		...
		try {
			const exists =
				...
				isNil(
					await this.knex.select('id').from('sigedin_fields').where({ collection, field: field.field }).first()
				) === false;

			// Check if field already exists, either as a column, or as a row in sigedin_fields
			if (exists) {
			...
			await this.knex.transaction(async (trx) => {
				const itemsService = new ItemsService('sigedin_fields', {
					knex: trx,
		...
		try {
			...
			await this.knex.transaction(async (trx) => {
				...
				if (Object.keys(collectionMetaUpdates).length > 0) {
					await trx('sigedin_collections').update(collectionMetaUpdates).where({ collection });
				}

				// Cleanup sigedin_fields
				...
				if (metaRow) {
					...
					await trx('sigedin_fields')
						.update({ group: null })
						.where({ group: metaRow.field, collection: metaRow.collection });
				}

				await trx('sigedin_fields').delete().where({ collection, field });
			});
