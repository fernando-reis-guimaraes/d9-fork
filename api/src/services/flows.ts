...
export class FlowsService extends ItemsService<FlowRaw> {
	...
	override async deleteMany(keys: PrimaryKey[], opts?: MutationOptions): Promise<PrimaryKey[]> {
		...
		// this is to prevent foreign key constraint error on sigedin_operations resolve/reject during cascade deletion
		await this.knex('sigedin_operations').update({ resolve: null, reject: null }).whereIn('flow', keys);
