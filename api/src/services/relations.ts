...
export class RelationsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.relationsItemService = new ItemsService('sigedin_relations', {
			knex: this.knex,
			schema: this.schema,
			// We don't set accountability here. If you have read access to certain fields, you are
			// allowed to extract the relations regardless of permissions to sigedin_relations. This
	...
	async createOne(relation: Partial<Relation>, opts?: MutationOptions): Promise<void> {
		...
		try {
			...
			await this.knex.transaction(async (trx) => {
				...
				const relationsItemService = new ItemsService('sigedin_relations', {
					knex: trx,
					schema: this.schema,
					// We don't set accountability here. If you have read access to certain fields, you are
					// allowed to extract the relations regardless of permissions to sigedin_relations. This
	...
	async deleteOne(collection: string, field: string, opts?: MutationOptions): Promise<void> {
		...
		try {
			await this.knex.transaction(async (trx) => {
				...
				if (existingRelation.meta) {
					await trx('sigedin_relations').delete().where({ many_collection: collection, many_field: field });
				}
	...
	private get hasReadAccess() {
		return !!this.accountability?.permissions?.find((permission) => {
			return permission.collection === 'sigedin_relations' && permission.action === 'read';
		});
	}
