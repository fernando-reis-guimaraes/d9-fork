...
export class FilesService extends ItemsService {
	...
	async uploadOne(
		...
		if ('folder' in payload === false) {
			const settings = await this.knex.select('storage_default_folder').from('sigedin_settings').first();
		...
		const sudoService = new ItemsService('sigedin_files', {
			knex: this.knex,
			schema: this.schema,
		});
	...
	async importOne(importURL: string, body: Partial<File>): Promise<PrimaryKey> {
		const fileCreatePermissions = this.accountability?.permissions?.find(
			(permission) => permission.collection === 'sigedin_files' && permission.action === 'create'
		);
