...
describe('run', () => {
	...
	describe('when passed the argument up', () => {
		it('returns "Nothing To Upgrade" if no sigedin_migrations', async () => {
			tracker.on.select('sigedin_migrations').response(['Empty']);
		...
		it('returns "Method implemented in the dialect driver" if no sigedin_migrations', async () => {
			tracker.on.select('sigedin_migrations').response([]);
		...
		it('returns undefined if the migration is successful', async () => {
			...
			tracker.on.delete('sigedin_relations').response([]);
			tracker.on.insert('sigedin_migrations').response(['Remove System Relations', '20201029A']);
	...
	describe('when passed the argument down', () => {
		it('returns "Nothing To downgrade" if no valid sigedin_migrations', async () => {
			tracker.on.select('sigedin_migrations').response(['Empty']);
