...
export async function up(knex: Knex): Promise<void> {
	...
	await knex('sigedin_relations').update({ one_deselect_action: 'nullify' });

	await helper.changeToType('sigedin_relations', 'sort_field', 'string', {
		length: 64,
	});

	await helper.changeToType('sigedin_relations', 'one_deselect_action', 'string', {
		nullable: false,
		default: 'nullify',
	});
}
