<template>
	...
	<div v-else class="many-to-many">
					...
					>
						...
						<v-menu show-arrow placement="bottom-end">
							...
							<v-list>
								...
								<v-list-item
									clickable
									:download="element.sigedin_files_id.filename_download"
		...
		>
			<template #actions>
				<v-button
					v-if="currentlyEditing !== '+' && relationInfo.relatedCollection.collection === 'sigedin_files'"
					secondary
					rounded
					icon
...
const downloadName = computed(() => {
	if (relatedPrimaryKey.value === null || relationInfo.value?.relatedCollection.collection !== 'sigedin_files') return;
	const junctionField = relationInfo.value.junctionField.field;
	const relationPkField = relationInfo.value.relatedPrimaryKeyField.field;

	return displayItems.value.find((item) => get(item, [junctionField, relationPkField]))?.sigedin_files_id
		?.filename_download;
});
