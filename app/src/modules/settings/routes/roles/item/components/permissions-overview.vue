...
export default defineComponent({
	...
	setup(props) {
		...
		const systemCollections = computed(() =>
			orderBy(
				collectionsStore.collections.filter((collection) => collection.collection.startsWith('sigedin_') === true),
				'name'
			)
		);
		...
		function useReset() {
			...
			async function resetSystemPermissions(useRecommended: boolean) {
				...
				const toBeDeleted = permissions.value
					.filter((permission) => permission.collection.startsWith('sigedin_'))
